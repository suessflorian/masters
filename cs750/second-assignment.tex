...
\subsubsection*{Lemma 1: $w_i \in w$, $w'_k \in \bm{R}(w)$, $k=|w|-i+1 \Rightarrow w_i = w_k$}
...
\subsubsection*{Lemma 2: $a,b \in \{0,1\}^*$ we have $\bm{R}(ab) = \bm{R}(b)\bm{R}(a)$.}
...
\subsubsection*{Lemma 3: $\bm{R}(\bm{R}(w)) = w$ for all $w \in \{0,1\}^*$.}
...
\end{align*}
... then crucially by \textbf{lemma 2} we continue to see;
\begin{align*}
